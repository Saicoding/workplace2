...
Page({
  ...
  onLoad: function () {
    ...
    //设置标题、标题背景颜色、tabbar样式
    share.setColor("jj",true,true);
  ...
  bindPickerChange: function (e) {
    ...
    if (user) {//如果有用户名
      wx.setStorageSync("jj_tiku_id" + user.username, {
  ...
  GOModelReal: function (e) {
    ...
    let kid = self.data.zhangjie_id;
    let url1 = '/pages/tiku/modelReal/modelRealList/modelRealList?kid=' + kid + "&ti=" + ti+"&category=jj";
  ...
  setZhangjie: function (res) {
    ...
    let user = wx.getStorageSync('user');
    let tiku = wx.getStorageSync("jj_tiku_id" + user.username);
